...
public class Maceta1_2 extends Actor implements Disposable {
    public static boolean var20= true;
...
float timeDurationTouch;
    ...
    public void act(float delta) {
        colisiones();
        if (MyGdxGame.Maceta1Colocadas.getInteger("Posiciones5") == 1 && MenuBuild.Maceta1Build) {
            for (Rectangle e : Tierra1.rects) {
                if (Jugador.jugador.overlaps(e)) {
                    if (noToca) {
                        MyGdxGame.Maceta1Colocadas.putInteger("Posiciones5", 2);
                        Maceta1_2.putFloat("X15", Jugador.body.getPosition().x);
                        if (AddResources.TouchConfirm) {
                            Maceta1_2.flush();
                        }
                        Maceta1_2.putFloat("Y15", (e.y + (5 / Pixels)) - 5 / Pixels);
                        if (AddResources.TouchConfirm) {
                            Maceta1_2.flush();
        ...
        if (MyGdxGame.Maceta1Colocadas.getInteger("Posiciones5") == 1 && MenuBuild.Maceta1Build) {
            ...
            if (!cambiarPosicion) {
                x = Maceta1_2.getFloat("X15");
                y = Maceta1_2.getFloat("Y15");
                Cuerpo2.set(x, y, 32/Pixels,32/Pixels);
            }
            ...
            for (Rectangle e : Tierra1.rects) {
                if (noToca) {
                    if (Jugador.jugador.overlaps(e)) {
                        ...
                        Maceta1_2.putFloat("Y15", (e.y + (5 / Pixels)) - 3 / Pixels);
                        if (AddResources.TouchConfirm) {
                            Maceta1_2.flush();
    ...
    {
        if(Cuerpo2.overlaps(Casa2Pisos.Cuerpo2)||Cuerpo2.overlaps(Casa2Pisos2.Cuerpo2)||Cuerpo2.overlaps(com.mygdx.game.actors.Casa1.Cuerpo2)||Cuerpo2.overlaps(Casa2.Cuerpo2)||Cuerpo2.overlaps(Maceta1_1.Cuerpo2)||Cuerpo2.overlaps(Maceta1_3.Cuerpo2)
        ...
        {
        AddResources.isFreeSpaceTop6= false;
    }else
    {
        AddResources.isFreeSpaceTop6 = true;
    }
    }
